...
function MyContributedPresales() {
  ...
  const fetchAllUserPools = async () => {
    ...
    try {
      if (account) {
        const endpoint = `${process.env.REACT_APP_API_BASE_URL}/api/v1/presale/get-contributed-pools/${account}?page=${currentPage}&limit=${PAGE_LIMIT}`
        const response = await axios.get(endpoint)
        if (response.status === 200) {
          const payload = response.data.payload
          console.log("NIPPA payload", payload)
          if (payload) {
...
export default MyContributedPresales